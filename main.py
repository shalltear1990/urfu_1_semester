...
def predict(item: Item):
    """Данный метод принимает в качестве параметра \"text\" в теле запроса текст, 
    который будет использован для создания аннотации с помощью предобученной модели МО 
    и возвращает json документ, в котором содержатся: 
    принятый к обработке текст, количество слов в этом тексте и сгенерированная аннотация"""
    return {
